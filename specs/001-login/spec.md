...
- 동시 로그인(같은 부서 비밀번호로 여러 브라우저) → **허용** (제한 없음, Auth Phase A 특성상 부서 공용 접근)
...
| 메뉴 | 경로 | 관리자 | 사용자 | 비고 |
|------|------|:------:|:------:|------|
| 자치법규관리 | `/ordinances` | O | O | 사용자는 소속 부서만 |
| 법령관리 | `/laws` | O | X | 상위법령 CRUD |
| 변경감지 | `/amendments` | O | X | 법령변경 동기화+승인 |
| 개정검토 | `/revision-needed` | O | O | 사용자는 소속 부서만 |
| 부서관리 | `/departments` | O | X | |
| 현황 | `/dashboard` | O | O | 사용자는 소속 부서 KPI |
...
- Q: `user_type` 값 네이밍은? → A: `USER` / `ADMIN`으로 통일. 기존 `DEPARTMENT`/`GENERAL` 네이밍 폐기
- Q: 동시 로그인(같은 부서 비밀번호로 여러 브라우저) 허용 여부? → A: **허용** (제한 없음). Auth Phase A는 부서 공용 접근이므로 세션 수 제한 불필요
